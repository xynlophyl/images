...
class ImageProcessor():
    ...
    def gaussian_blur(self, image: np.ndarray, std_dev: float = 1.0, stride: int = 1, mode: str = "convolve") -> np.ndarray:
        kernel_size = int(std_dev*3)     
        center_x = center_y = kernel_size//2
        gaussian_mask = np.array([[self._gaussian(x-center_x, y-center_y, std_dev) for x in range(kernel_size)] for y in range(kernel_size)])
